...
function jwtGeneration(sId, sEmail, sSub, sAdmin) {
  //JWT
  const jwtSecret = fs
    .readFileSync("/etc/secrets/SECRET_KEY_JWT", "utf8")
    .trim();
  // Generate JWT
  const token = jwt.sign(
    { _id: sId, email: sEmail, sub: sSub, admin: sAdmin },
    jwtSecret
  );
  return token;
}
...
router.post("/signup", async (req, res) => {
  ...
  if (error) {
    return res
      .status(400)
      .send(`${error.details[0].message}, validation failed`);
  }

  // Check if the user already exists by email or sub
  const existingUser = await User.findOne({
    $or: [{ email: req.body.email }, { sub: req.body.sub }],
  });
  if (existingUser) {
